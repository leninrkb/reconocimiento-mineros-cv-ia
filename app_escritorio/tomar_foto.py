...
"border-radius: 10px;")
    ...
    def tomar_foto(self):
        ret, frame = cap.read()
        ancho_deseado = 180 # especificar el ancho deseado
        alto_deseado = 180 # especificar el alto deseado
        imagen_redimensionada = cv2.resize(frame, (ancho_deseado, alto_deseado))
        ...
        cv2.imwrite(carpeta+today+'-'+hour+minut+seconds+microseconds+extencion, imagen_redimensionada)
    
        cursor = cnx.cursor()

        # Insertar una fila en la tabla "personas"
        query = "INSERT INTO registro_empleado (CED_EMP, FECHA_ENTRADA, CASCO, CHALECO, BOTAS, OBSERVACION, PATH) VALUES (%s, %s, %s,%s,%s,%s,%s)"
        values = ("1804567891", dt, "1", "1", "1", "ninguna", PATH)
